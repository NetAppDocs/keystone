...
=== Capacity units changed in APIs
All storage capacity units and calculations in NetApp Service Engine APIs are converted to GiB, TiB, and PiB from GB, TB, and PB respectively. This is in keeping with the units displayed on the existing user interface.
