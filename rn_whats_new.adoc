...
Navigate to this screen from the *Monthly Charges* widget on the dashboard or from *ADMINISTRATION > Billing* to view a monthly, subscription-level break up of the charges for all your subscribed services. The billing data is based on your committed and burst capacity usage, and is available for the previous months, with the projected billing for the current month.
For more information, see link:sewebiug_billing.html[View billing]

=== New Alerts screen
A new *Alerts* screen is introduced in this release that enables you to generate custom alerts. By using this screen, you can create alert messages for critical events pertaining to your environment, and convey them to other users. They can view and dismiss the alerts, as required.
[NOTE]
These messages are custom alerts, and not related with the automatic, system-generated alerts.

For more information, see link:sewebiug_alerts.html[Create and manage alerts]

=== Enhanced user interface
The folloiwng enhancements have been made on the NetApp Service Engine user interface:
* The *Dashboard* menu on the on the left navigation pane presents an intuitive navigation point to access the dashboard.
