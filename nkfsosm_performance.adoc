...
[.lead]
NetApp Keystone offers capacity and capacity at predefined performance service levels (service levels). Each service level is defined by its I/O density, which is the ratio of performance (input/output operations per second [IOPS]) and used storage (TiB of stored data) which is IOPS/TiB.
