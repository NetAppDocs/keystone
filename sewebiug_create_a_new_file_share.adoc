...
* For creating a CIFS or NFS file share, the server must be enabled for the respective service. For a multiprotocol file shares, the server should support both CIFS and NFS services.
...
+
. Select the share type: NFS, CIFS, or Multiprotocol. For creating
