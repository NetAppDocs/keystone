...
. On Digital Advisor, click *View More Details* on the widget to view the usage details and alerts on your volumes on the *Keystone Subscription* page. Alternately, from the left navigation pane, go to *GENERAL > Keystone Subscription*.
...
+
The bar charts appear in these colors to indicate the following consumed capacity as defined within the service level:
...
You can view the performance details in the following sections based on the combination of the subscription number, volume name, and the date range selected. The details are displayed as per service level assigned to the volume. You can see the cluster name, and the volume type, for example, the read and write permissions assigned to it. Any warning message associated with the volume is also displayed.

[NOTE]
The retrieved details are based on the service level objective for each service level. For example, the peak IOPS, max throughput, target latency, and other metrics are determined by the individual settings for the service level. For more information about the settings, see link:nkfsosm_performance.html[Service levels]. 

The performance data displayed on the horizontal graph is averaged out over every 5 minutes and arranged as per the date range of the query. You can scroll across the graphs and hover your mouse over specific data points to drill further down into the collected data. You can view the provisional data when the exact usage data is unavailable at that point and is substituted by the data available at the previous data collection point.

=== IOPS/TiB
This section displays the input output graphs for the workloads of the volume based on the date range of the query. The peak IOPS for the service level and the current IOPS (in the last five minutes, not based on the date range of the query) are displayed, along with the minimum, maximum, and average IOPS for the time range, in IOPS/TiB .

image:perf-iops.png[]

=== Throughput (MBps/TiB)
This section displays the throughput graphs for the workloads of the volume based on the date range of the query. The max throughput as per the service level (SLO Max), and current throughput (in the last five minutes, not based on the date range of the query) are displayed, along with the minimum, maximum, and average throughput for the time range, in MBps/TiB.

image:perf-thr.png[]

=== Latency (ms)
This section displays the latency graphs for the workloads of the volume based on the date range of the query. The max latency as per the service level (SLO Max), and current latency (in the last five minutes, not based on the date range of the query) are displayed, along with the minimum, maximum, and average latency for the time range, in milliseconds.

image:perf-lat.png[]

=== Logical Used (TiB)
This section displays the provisioned and the logical used capacities of the volume. The current logical used capacity (in the last five minutes, not based on the date range of the query), and the minimum, maximum, and average usage for the time range are displayed in TiBs.
