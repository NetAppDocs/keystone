...
[%collapsible]
====
If you have subscribed to the Data Protection (DP) service, you can view the breakup of the consumption data according to the MetroCluster primary and mirror sites on the *Subscription Time Series* tab. 
...
For the _Data-Protect Extreme_ service level and the same data collection point, the chart appears like this:
...
+
The volume details are displayed. You can scroll across the columns and learn more about them by hovering your mouse on the information icons beside the column headings. You can sort by the columns and filter the lists to view specific information. For Data Protection services, an additional column appears to indicate whether the volume is a primary or mirror volume in the MetroCluster configuration. You can copy individual node serial numbers by clicking the *Copy Node Serials* button.
